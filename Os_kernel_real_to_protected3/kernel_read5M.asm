; 将一段数据写到5M的内存地址上,再读取写入的数据输出屏幕
; 寄存器指向GDT某个描述符索引,获得基地址,基地址(4字节32位)再加上偏移
; nasm -o kernel.bat kernel_read5M.asm
%include "pm.inc"
...
write_msg_to_5M:  ;将si指向的字符一个个写到5M内存处
    ...
    mov   al, [si]
    mov   [es:edi], al  ; es存储的是LABEL_DESC_5M在GDT中的偏移
    add   edi, 1        ; edi从0开始一直累加将内容一直写下去
    add   si, 1
    jmp   write_msg_to_5M
