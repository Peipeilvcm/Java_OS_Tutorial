...
struct MEMMAN* memman  = (struct MEMMAN*)0x100000; //设置内存管理器的地址
...
void CMain(void)
{
    initBootInfo(&bootInfo); //初始化显存和设置背景
    char *vram = bootInfo.vgaRam;
    xsize = bootInfo.screenY,ysize = bootInfo.screenY;
    struct SHTCTL *shtctl; //*图层控制器
    struct SHEET *sht_back = 0,*sht_mouse = 0;//鼠标的图层和背景图层

    fifo8_init(&keyinfo,32,keybuf); //给键盘32位的缓冲区
    fifo8_init(&mouseinfo,128,mousebuf);//给鼠标128位的缓冲区

    init_palette();//初始化调色板
    init_keyboard();//初始化键盘

    int memCnt = get_memory_block_count(); //计算物理内存块,检测内存信息

    struct AddrRangeDesc* memDesc = (struct AddrRangeDesc*)get_adr_buffer();//把内存转换为内存描述符的形式
    memman_init(memman); //初始化内存管理器
    memman_free(memman,0x001008000,0x3FEE8000);//释放对应首地址的大小的内存

    shtctl = shtctl_init(memman,vram,xsize,ysize);//*初始化一个图层控制器
    sht_back = sheet_alloc(shtctl); //*分配一个图层给背景
    sht_mouse = sheet_alloc(shtctl); //分配一个图层给鼠标
    buf_back = (unsigned char*)memman_alloc_4k(memman,xsize*ysize); //分配一个4K大小的空间给背景，返回这个内存的首地址
    
    sheet_setbuf(sht_back,buf_back,xsize,ysize,COLOR_INVISIBLE); //设置图层背景的形状和颜色
    sheet_setbuf(sht_mouse,buf_mouse,16,16,COLOR_INVISIBLE); //设置鼠标图层的形状和颜色

    init_screen8(buf_back,xsize,ysize);  //初始化背景颜色
    sheet_slide(shtctl,sht_back,0,0); //移动背景的左上角位置到(0,0)

    mx = (xsize - 16) / 2;
    my = (ysize - 28 - 16) / 2;
    sheet_slide(shtctl,sht_mouse,mx,my); //移动鼠标

    sheet_updown(shtctl,sht_back,0);  //背景图层移动到最底层
    sheet_updown(shtctl,sht_mouse,1); //鼠标图层移动到第一层

    io_sti();//开中断,接收键盘鼠标的中断信号
    enable_mouse(&mdec);
    ...
    for(;;)
    {
        ...
        else if(fifo8_status(&keyinfo) != 0)
        {
            ...
            if(data == 0x1C)  //回车键
            {
                showMemoryInfo(memDesc + count,buf_back,count,xsize,COL8_FFFFFF);
                count = (count + 1);
                    if(count > memCnt)
                    {
                        count = 0;
                    ...
                    sheet_refresh(shtctl);//绘制图层
            }
        }
            else if(fifo8_status(&mouseinfo) != 0)
            {
                    show_mouse_info(shtctl,sht_mouse);//*根据鼠标的中断绘制鼠标的图层
            }
