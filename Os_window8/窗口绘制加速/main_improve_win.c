...
void CMain(void)
{
    ...
    struct AddrRangeDesc* memDesc = (struct AddrRangeDesc*)get_adr_buffer(); //内存描述符
    memman_init(memman);
    ...
    buf_back = (unsigned char*)memman_allock_4k(memman,xsize*ysize);

    sheet_setbuf(sht_back,buf_back,xsize,ysize,COLOR_INVISIBLE); //背景图层设置
    sheet_setbuf(sht_mouse,buf_mouse,16,16,COLOR_INVISIBLE); //鼠标图层设置

    init_screen8(buf_back,xsize,ysize); //背景形状颜色填充

    init_mouse_cursor(buf_mouse,COLOR_INVISIBLE); //鼠标形状颜色填充到buf_mouse中
    sheet_slide(shtctl,sht_back,0,0); //背景图层移动到左上角(0,0)
...
void  show_mouse_info(struct SHTCTL *shtctl, struct SHEET *sht_back,struct SHEET *sht_mouse) {
    ...
    if (mouse_decode(&mdec, data) != 0) {
         computeMousePosition(shtctl, sht_back, &mdec);//重新计算鼠标的位置，保存在静态变量mx,my的坐标中
        
         sheet_slide(shtctl, sht_mouse, mx, my); //移动鼠标到mx,my的位置中
    }
}
